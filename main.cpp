...
void screen()
{
    ...
    drawstring(230,425,0.0,"Nitte Mahalinga Adyanthaya Memorial Institute of Technology",1);
    glColor3f(0.0,0.0,0.0);
    drawstring(200,385,0.0,"DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING",1);
    glColor3f(0.6,1.0,0.0);
    drawstring(260,350,0.0,"A MINI PROJECT ON COMPUTER GRAPHICS ",1);
    glColor3f(1.0,1.0,0.0);
    drawstring(340,300,0.0,"AQUARIUM USING OPENGL",1);
    glColor3f(1.0,0.0,0.0);
    drawstring(135,205,0.0," STUDENT NAME             USN",0);
    glColor3f(0.0,0.0,0.0);
    drawstring(140,170,0.0,"Sinha Shruti Shashi-4NM20CS180",0);
    drawstring(140,135,0.0,"Swarnil Sanjeev Sinha-4NM20CS194",0);
    glColor3f(1.0,0.0,0.0);
    drawstring(710,205,0.0,"Under the Guidance of",0);
    glColor3f(0.0,0.0,0.0);
    drawstring(720,170,0.0,"Prof Puneeth R P",0);
    glColor3f(1.0,1.0,0.0);
    drawstring(406,95,0.0,"Academic Year 2022-23",0);
...
void fish2()
{
    ...
    // First triangle
    glBegin(GL_TRIANGLES);
    glColor3ub(rand()%500,rand()%500, 0.0);
...
void pb4()
{
    ...
    {
        glColor3ub(142, 20, 30);
        y =(sin(angle*pi/180)*42);
        x =(cos(angle*pi/180)*80);
        glVertex2f(x+720,y+12);
    }
    glEnd();
}
void pb3()
...
void mov(void)
{
    int p = 300; // Set initial x coordinate for red fish
    int q = 200;
    if(a>=-500)
        a=a-7;
    else
    ...
    // Check if blue fish collides with shark
    if (a < s && (s-a) < 100)
    {
        a = -1000; // Blue fish disappears
    }
    // Check if red fish collides with shark
    if (aa < s && (s-aa) < 100)
    {
        aa = -1000; // Red fish disappears
    }
